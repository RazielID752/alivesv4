      ...
      >
        <nav className="grid gap-1 p-3" aria-label="Navegacao mobile">
          {navigation.map((item) => {
            ...
            return (
              <Link
                className="group flex items-center gap-3 rounded-2xl px-4 py-3 text-sm text-zinc-300 transition hover:bg-white/5 hover:text-blue-600"
                href={item.href}
                key={item.href}
              ...
              >
                <Icon
                  aria-hidden="true"
                  className="h-4 w-4 text-white/55 transition group-hover:text-blue-600"
                />
                {item.label}
              </Link>
            );
